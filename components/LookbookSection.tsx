...
export default function LookbookSection() {
    return (
        <section className="py-16 md:py-24 px-4 sm:px-6 md:px-12 bg-stone-50">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-center mb-10 md:mb-16 gap-4 md:gap-6">
                    <div className="text-center md:text-left">
                        <h2 className="text-3xl sm:text-4xl md:text-6xl font-black text-stone-900 tracking-tight leading-none">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
                    {lookbookData.map((lookbook, index) => (
                        <Link key={index} href={`/lookbook/${index}`} className="group relative aspect-[3/4] overflow-hidden rounded-2xl sm:rounded-[2.5rem] bg-stone-200 shadow-lg sm:shadow-2xl transition-all duration-500 hover:-translate-y-2">
                            {/* Image Background */}
                            <img
                                src={lookbook.image}
                            ...
                            <div className="absolute inset-0 p-4 sm:p-8 flex flex-col justify-end">
                                <div className="translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                                    <div className="text-white font-black text-base sm:text-2xl mb-1">{lookbook.celebrity}</div>
                                    <div className="text-white/60 text-[10px] sm:text-sm font-bold uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity duration-500">
